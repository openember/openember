...
TemperatureEndpoint::TemperatureEndpoint(EndpointConfig config, EndpointContext context)
    : config_(std::move(config)),
      context_(std::move(context)),
      temperature_(openember::sensor::MockTemperatureOptions{
          config_.endpoint_id,
          config_.frame_id,
...
const std::string& TemperatureEndpoint::Id() const {
    return config_.endpoint_id;
}

bool TemperatureEndpoint::Configure() {
...
bool TemperatureEndpoint::Start() {
    auto result = temperature_.Start();
    if (!result.Ok()) {
    ...
    publisher_ =
        context_.node->Advertise<openember::msgs::sensor::v1::TemperatureSample>(
            config_.topic);
    running_.store(true);
    thread_ = std::thread(&TemperatureEndpoint::Run, this);
...
void TemperatureEndpoint::Stop() noexcept {
...
EndpointStatus TemperatureEndpoint::Status() const {
    std::lock_guard<std::mutex> lock(status_mutex_);
    return status_;
}

void TemperatureEndpoint::Run() {
    while (running_.load() && openember::Ok()) {
        auto result = temperature_.ReadSample(std::chrono::milliseconds(600));
        if (!result.Ok()) {
        ...
        if (!publisher_.Publish(msg)) {
            RecordError("failed to publish temperature sample");
            continue;
        }
...
void TemperatureEndpoint::RecordError(const std::string& message) {
    std::lock_guard<std::mutex> lock(status_mutex_);
...
void TemperatureEndpoint::SetError(const std::string& message) {
    std::lock_guard<std::mutex> lock(status_mutex_);
