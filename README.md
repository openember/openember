...
**OpenEmber** 是一个面向常见智能设备的嵌入式应用软件开发框架，目前仅支持 Linux 操作系统。核心以 **C++ 为主线**，同时通过稳定的 **C ABI 兼容接口**尽量照顾嵌入式/传统生态的集成需求。和 ROS2 类似，OpenEmber 也是分布式架构，节点模型可通过“发布-订阅（pub/sub）”机制进行消息通信。
