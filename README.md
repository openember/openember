...
**OpenEmber** 是一个面向常见智能设备的嵌入式应用软件开发框架，目前仅支持 Linux 操作系统，核心使用 C/C++ 开发。和 ROS2 类似，OpenEmber 也是分布式架构，节点模型可通过“发布-订阅（pub/sub）”机制进行消息通信。
...
1. 一个仓库同时支持「纯 C」与「现代 C++」两套工程路线，用户可以根据项目需要生成符合 MISRA-C 的工程，以及现代 C++ 风格的工程。

   API 兼容等问题通过本仓库的工程结构和工具链设计来解决，例如顶层的 CMake 设计：
   ...
   提供一个 OpenEmber 脚手架工具，如 `ember new`：
...
3. 依赖库管理：第三库不依赖系统安装，默认使用 FetchContent 方式，同时支持完全内置第三方库（企业/离线场景），可通过配置切换模式。
