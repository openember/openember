...
# Eclipse Zenoh C / C++（https://github.com/eclipse-zenoh/zenoh-c / zenoh-cpp）
set(OPENEMBER_ZENOHC_VERSION "1.9.0")
set(OPENEMBER_ZENOHC_URL
    "https://github.com/eclipse-zenoh/zenoh-c/archive/refs/tags/${OPENEMBER_ZENOHC_VERSION}.tar.gz")

set(OPENEMBER_ZENOHCXX_VERSION "1.9.0")
set(OPENEMBER_ZENOHCXX_URL
    "https://github.com/eclipse-zenoh/zenoh-cpp/archive/refs/tags/${OPENEMBER_ZENOHCXX_VERSION}.tar.gz")

# zenoh-c 上游 rust-toolchain.toml 钉版本；空=自动检测，损坏时回退 +stable
set(OPENEMBER_ZENOHC_CARGO_CHANNEL "" CACHE STRING
    "Override zenoh-c Cargo channel (e.g. +stable, +1.93.0). Empty = auto from rust-toolchain.toml")
...
set(OPENEMBER_ZENOHC_CACHE_KEY "zenoh-c-${OPENEMBER_ZENOHC_VERSION}")
set(OPENEMBER_ZENOHC_STAGE_DIR_NAME "${OPENEMBER_ZENOHC_CACHE_KEY}")
set(OPENEMBER_ZENOHCXX_CACHE_KEY "zenoh-cpp-${OPENEMBER_ZENOHCXX_VERSION}")
set(OPENEMBER_ZENOHCXX_STAGE_DIR_NAME "${OPENEMBER_ZENOHCXX_CACHE_KEY}")
...
function(openember_transport_resolve_zenoh_c)
    if(OPENEMBER_THIRD_PARTY_MODE STREQUAL "SYSTEM")
        find_package(zenohc QUIET)
        if(NOT zenohc_FOUND)
            message(FATAL_ERROR "zenohc not found but OPENEMBER_THIRD_PARTY_MODE=SYSTEM")
        endif()
        if(NOT TARGET zenohc::lib)
            message(FATAL_ERROR "zenohc package found but zenohc::lib target is missing")
        endif()
    ...
    if(OPENEMBER_THIRD_PARTY_MODE STREQUAL "FETCH" OR OPENEMBER_THIRD_PARTY_MODE STREQUAL "VENDOR")
        if(NOT OPENEMBER_THIRD_PARTY_BUNDLE_ZENOH_C)
            message(FATAL_ERROR
                "OPENEMBER_THIRD_PARTY_BUNDLE_ZENOH_C=OFF: install zenoh-c or enable bundle (Third party).")
        endif()
    endif()

    openember_get_zenoh_c()
    set(OPENEMBER_ZENOHC_LIBRARIES ${OPENEMBER_ZENOHC_LIBRARIES} PARENT_SCOPE)
endfunction()

function(openember_transport_resolve_zenoh_cpp)
    if(OPENEMBER_THIRD_PARTY_MODE STREQUAL "SYSTEM")
        find_package(zenohcxx QUIET)
        if(NOT zenohcxx_FOUND)
            message(FATAL_ERROR "zenohcxx not found but OPENEMBER_THIRD_PARTY_MODE=SYSTEM")
        endif()
        if(NOT TARGET zenohcxx::zenohc)
            message(FATAL_ERROR "zenohcxx package found but zenohcxx::zenohc target is missing")
        endif()
    ...
    if(OPENEMBER_THIRD_PARTY_MODE STREQUAL "FETCH" OR OPENEMBER_THIRD_PARTY_MODE STREQUAL "VENDOR")
        if(NOT OPENEMBER_THIRD_PARTY_BUNDLE_ZENOHCXX)
            message(FATAL_ERROR
                "OPENEMBER_THIRD_PARTY_BUNDLE_ZENOHCXX=OFF: install zenoh-cpp or enable bundle (Third party).")
        endif()
    endif()

    if(NOT TARGET zenohc::lib)
        openember_transport_resolve_zenoh_c()
    endif()
    openember_get_zenoh_cpp()
...
function(openember_third_party_resolve_optional_cxx_deps)
    ...
    if(OPENEMBER_WITH_ZENOHCXX)
        openember_transport_resolve_zenoh_cpp()
    endif()
endfunction()
...
function(openember_third_party_prefetch_unused_bundles)
    ...
    if(OPENEMBER_THIRD_PARTY_BUNDLE_NNG)
    ...
    if(OPENEMBER_THIRD_PARTY_BUNDLE_LCM)
    ...
    if(OPENEMBER_THIRD_PARTY_BUNDLE_LIBZMQ)
    ...
    if(OPENEMBER_THIRD_PARTY_BUNDLE_CPPZMQ)
    ...
    if(NOT DEFINED OPENEMBER_COMPONENT_TRANSPORT OR OPENEMBER_COMPONENT_TRANSPORT)
        set(_zenoh_c_needed ON)
    endif()
    if(OPENEMBER_THIRD_PARTY_BUNDLE_ZENOH_C AND NOT _zenoh_c_needed)
        if(NOT OPENEMBER_ZENOHC_LOCAL_SOURCE)
            openember_prepare_zenoh_c_source(_unused)
            message(STATUS "Third-party: pre-fetch only (not built): zenoh-c")
        endif()
    endif()

    set(_zenohcxx_needed OFF)
    if(OPENEMBER_WITH_ZENOHCXX)
        set(_zenohcxx_needed ON)
    endif()
    if(NOT DEFINED OPENEMBER_COMPONENT_TRANSPORT OR OPENEMBER_COMPONENT_TRANSPORT)
        set(_zenohcxx_needed ON)
    endif()
    if(OPENEMBER_THIRD_PARTY_BUNDLE_ZENOHCXX AND NOT _zenohcxx_needed)
        if(NOT OPENEMBER_ZENOHCXX_LOCAL_SOURCE)
            openember_prepare_zenoh_cpp_source(_unused)
            message(STATUS "Third-party: pre-fetch only (not built): zenoh-cpp")
        endif()
    endif()
endfunction()
