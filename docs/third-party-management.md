...
| [zenoh-c](https://github.com/eclipse-zenoh/zenoh-c) | `OPENEMBER_ZENOHC_VERSION` | `GetZenohC.cmake` | `OPENEMBER_COMPONENT_TRANSPORT=ON`（默认）等；需 **Rust/Cargo** |
| [zenoh-cpp](https://github.com/eclipse-zenoh/zenoh-cpp) | `OPENEMBER_ZENOHCXX_VERSION` | `GetZenohCpp.cmake` | Transport 组件（`openember::transport`）；依赖 zenoh-c |
